...
| infra     | Data centres coloured by function (ai training, hyperscale, colo, government, crypto, edge); the power grid (plants by fuel, 230 kV+ substations, HV lines); water sources; curated chip fabs and raw-material chokepoints; a siting-pressure prediction heatmap; supply-chain links | OpenStreetMap; curated public facts (~2025)             |
| astro     | Astrocartography: planetary lines over the Earth for a chosen date, local time and time zone — each body’s MC/IC meridians and AC/DC rising/setting curves, coloured per body                                                                                                        | compact planetary theory (in-browser)                   |
...
**INFRA prediction is a model, not a fact.** The infra panel scores each cell of the view for data-centre siting pressure from the live signals on screen — nearness to 230 kV+ substations and to existing clusters — and draws it as a distinct red heatmap. It is a transparent weighted heuristic, not a trained or backtested model, and never uses the same symbol as a confirmed site. The supply-chain links are a descriptive graph (solid where co-located, dashed where inferred); ADAM does not rank which shared substation, plant or aquifer to disable.
