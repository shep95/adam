 ...
 *
 *   ops rail        BRIEF (B) · ALERTS (A) · FILTER (G) · KEYS (?)
...
export function installOpsDeck({
  ...
  for (const [id, view] of Object.entries(VIEWS)) {
    const btn = button(
      ...
      () => toggleView(id),
      { 'aria-pressed': 'false', title: `${view.label} (${view.key})` },
    );
    btn.append(el(doc, 'span', 'adam-ops-rail-label', view.label));
    btn.append(el(doc, 'kbd', 'adam-ops-rail-key', view.key));
  ...
  root.append(lastCard, rail, flyout, pinRail, alertStack, overlay);
